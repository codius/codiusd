...
process.env.CODIUS_COST_PER_MONTH = '13'
...
describe('Config testing when process.env.CODIUS_COST_PER_MONTH is set', () => {
  beforeEach(function () {
  ...
  it('Validates Config does not break changes when process.env.CODIUS_COST_PER_MONTH is set', done => {
    const request = {
      method: 'GET',
      url: '/info'
    }
    if (server) {
      server.inject(request).then(response => {
        ...
        assert.strictEqual(res.costPerMonth, 13)
